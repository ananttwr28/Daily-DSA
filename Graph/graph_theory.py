...
if __name__ == "__main__":			
    v = 4      
    graph = [[] for _ in range(v)]     # 2. create list of size (v) of lists

    create_graph(graph)     # 3. call create fn and pass list graph

    # 5. for finding and printing neighbours of a vertex

    for i in range(0, len(graph[2])):       # 6. TC is O(no. of neighbours), run a loop till vertex/ index 2 and store it in edge which will be of type obj. and will access src and dest.
        edge = graph[2][i]       
